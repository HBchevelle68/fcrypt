...
int main(int argc, char* argv[]){
	...
	if(argc < 5 && argv[2][1] != 'h'){
		usage();
		return 0;
	}

	if(argv[2][0] == '-' && argv[2][1] == 'e' &&
	   argv[3][0] == '-' && argv[3][1] == 'p' && argv[4] != NULL){
		...
		if(fte.is_open()){
			...
			argv[3][0] == '-' && argv[3][1] == 'p' && argv[4] != NULL){ //Decyption	
		...
		if(hfile){
			std::string hash;
			FCrypt::Hash::FileHash(hfile, hash);
			std::cout << std::setw(10) << std::left << "SHA2-256: " << hash << "   " << argv[1] << std::endl;
		}
		else {
			std::cout << "Error: no file " << argv[1] << " found\n" << std::endl;
		}		
...
void usage() {
	...
	std::cout << std::setw(10) << std::left << "  -h"  << "Get SHA2 256/384/512 hash of file" << std::endl;
	...
	std::cout << "\nPassword:" << std::endl;
	std::cout << std::setw(10) << std::left << "  -p"  << "Password for file in order to encrypt or decrypt. Not used for hash action" << std::endl;
}
